@extends('index')
@section('content')
<form method="POST" action="{{route('students.update',['id' => $student])}}" enctype="multipart/form-data>
    @csrf
    @method('PUT')
    <fieldset>
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="image" class="form-label">{{ trans('message.students.image') }}</label>
            <input type="file" id="image" class="form-control @error('image') is-invalid @enderror" name="image" value="{{$student->image}}">
            @error('image')
            <div class="invalid-feedback">{{$message}}</div>
            @enderror
        </div>
